...
class SpikeResponseModel():
    ...
    def simulate(self, T, input):
        ...
        def sigma(s, t):
            # eq. 4.36
            def subfunction(x):
                return (m.exp(-1*x/self.tau_m))*(1/self.tau_s)*m.exp((x-t)/self.tau_s)*Hsf(t-x)
        ...
        def u_i(t, i):
            # eq. 4.34
            ti = np.random.binomial(n=1)
            tf = input[i]
            
            n = 0
            for j in range(len(input[i])):
                n = n+sigma(t-ti, t-tf[j])
            
            def subf(x):
                return Kai(t-ti, x)*(1/self.tau_s)*m.exp((x-t)/self.tau_s)*Hsf(t-x)
            
            value, error = scipy.integrate.quad(subf, 0, np.inf)
            
            return fai(t-ti) + n + value
        ...
        for s in range(self.n_inputs):
            print('the %th neuron:')
            for k in range(T/self.dt):
                print(pspike(u_i(self.dt*k, s)))